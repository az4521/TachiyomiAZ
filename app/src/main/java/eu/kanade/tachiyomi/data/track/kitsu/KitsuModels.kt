...
class KitsuSearchManga(obj: JsonObject) {
    ...
    private var startDate =
        obj["startDate"]?.jsonPrimitive?.content?.let {
            val outputDf = SimpleDateFormat("yyyy-MM-dd", Locale.US)
            outputDf.format(Date(it.toLong() * 1000))
        }
    private val endDate = obj["endDate"]?.jsonPrimitive?.content
...
class KitsuLibManga(obj: JsonObject, manga: JsonObject) {
    ...
    val type = manga["attributes"]!!.jsonObject["mangaType"]?.jsonPrimitive?.content.orEmpty()
    ...
    private val startDate = manga["attributes"]!!.jsonObject["startDate"]?.jsonPrimitive?.content.orEmpty()
    private val libraryId = obj["id"]!!.jsonPrimitive.int
    val status = obj["attributes"]!!.jsonObject["status"]!!.jsonPrimitive.content
    private val ratingTwenty = obj["attributes"]!!.jsonObject["ratingTwenty"]?.jsonPrimitive?.content
