...
struct ExternalSourceInfo: Codable, Hashable {
    ...
    let contentRating: ExtensionRunner.SourceContentRating?
    let altNames: [String]?
    let baseURL: String?
    ...
    var resolvedContentRating: ExtensionRunner.SourceContentRating {
        if let contentRating {
            contentRating
        } else if let nsfw, let rating = ExtensionRunner.SourceContentRating(rawValue: nsfw) {
            rating
        } else {
            .safe
        }
