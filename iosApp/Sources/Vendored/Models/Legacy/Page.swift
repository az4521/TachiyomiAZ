...
extension Page {
    func toNew() -> ExtensionRunner.Page {
        let content: ExtensionRunner.PageContent = if let imageURL, let url = URL(string: imageURL) {
            .url(url: url, context: context)
        } else if let text {
            .text(text)
        } else if let image {
#if os(macOS)
            .image(ExtensionRunner.PlatformImage(image))
#else
            .image(image)
#endif
