...
extension ComicInfo {
    static func load(manga: ExtensionRunner.Manga, chapter: ExtensionRunner.Chapter) -> ComicInfo {
    ...
    func toManga() -> ExtensionRunner.Manga? {
        ...
        let basicManga = ExtensionRunner.Manga(sourceKey: sourceKey, key: key, title: series ?? "")
        return copy(into: basicManga)
    }

    func toChapter() -> ExtensionRunner.Chapter? {
        guard
            let extraData = extraData(),
        ...
        let basicChapter = ExtensionRunner.Chapter(key: key)
        return copy(into: basicChapter)
    }

    func copy(into manga: ExtensionRunner.Manga) -> ExtensionRunner.Manga {
    ...
    func copy(into chapter: ExtensionRunner.Chapter) -> ExtensionRunner.Chapter {
        ...
        let url = web?.split(separator: " ", maxSplits: 1).first.flatMap { URL(string: String($0)) }
        return ExtensionRunner.Chapter(
            key: chapter.key,
