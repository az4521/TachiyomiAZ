...
extension String {
    ...
    ///
    /// `FilterGroupCreateView` calls this, but nothing in tachiyomiazios declares it -- the name is
    /// SwiftSoup's, whose `String.trim()` is only visible in files that import it, and that view
    /// imports only SwiftUI. Added here so the vendored view keeps its own spelling; the behaviour
    /// is the obvious one, and every call site uses it to normalise a title before comparison.
    func trim() -> String {
