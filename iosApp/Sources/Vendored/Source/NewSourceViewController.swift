...
class NewSourceViewController: UIViewController {
    ...
    private var listings: [ExtensionRunner.Listing] = [] {
        didSet {
    ...
    private var filters: [ExtensionRunner.Filter]? {
        didSet {
            updateHostingControllers()
        }
    }
    ...
    private var listingsBinding: Binding<[ExtensionRunner.Listing]> {
        .init(
    ...
    private var filtersBinding: Binding<[ExtensionRunner.Filter]?> {
        .init(
    ...
    struct ImportFileView: View {
        ...
        var body: some View {
            // Local file import is parked -- see Vendored/_excluded/Local -- and this sheet is
            // only reachable from the local source, which does not exist here.
            EmptyView()
        }
    }
    ...
    func configure() {
        ...
        if source.id == "local" /* local sources parked; see Vendored/_excluded/Local */ {
...
extension NewSourceViewController {
    ...
    private func showImportSheet() {
        // See importFileView: local file import is parked, so there is no sheet to present.
    }
}
