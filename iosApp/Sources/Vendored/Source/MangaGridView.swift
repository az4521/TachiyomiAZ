...
struct MangaGridView: View {
    ...
    let entries: [ExtensionRunner.Manga]
    @Binding var bookmarkedItems: Set<String>
    var loadMore: (() async -> Void)?
    var onSelect: ((ExtensionRunner.Manga) -> Void)?
    ...
    init(
        ...
        entries: [ExtensionRunner.Manga],
        bookmarkedItems: Binding<Set<String>> = .constant([]),
        loadMore: (() async -> Void)? = nil,
        onSelect: ((ExtensionRunner.Manga) -> Void)? = nil
    ) {
        self.source = source
    ...
    private func mangaGridItem(entry: ExtensionRunner.Manga) -> some View {
