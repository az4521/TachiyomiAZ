...
extension HistoryView {
    ...
    class ViewModel: ObservableObject {
        ...
        @Published var mangaCache: [String: ExtensionRunner.Manga] = [:]
        @Published var chapterCache: [String: ExtensionRunner.Chapter] = [:]
    ...
    private func setUpNotifications() {
            ...
            .sink { [weak self] output in
                ...
                Task { @MainActor in
                    if chapters.count == 1, let chapter = chapters.first {
                        ...
                        let chapterCacheKey = chapter.sourceKey + "." + chapter.mangaKey + "." + chapter.chapterKey
                        if
                            self.chapterCache[chapterCacheKey] != nil
                                || self.missingMangaQueue[MangaKey(sourceId: chapter.sourceKey, mangaId: chapter.mangaKey)] != nil
                        {
                ...
                Task { @MainActor in
                    if let chapters = output.object as? [ChapterIdentifier] {
                        for chapter in chapters {
                            let chapterId = chapter.sourceKey + "." + chapter.mangaKey + "." + chapter.chapterKey
                            self.removeStoredHistory(chapterCacheKey: chapterId)
                        }
                    } else if let manga = output.object as? MangaIdentifier {
                        self.removeStoredHistory(mangaCacheKey: manga.sourceKey + "." + manga.mangaKey)
                    }
                }
            }
            .store(in: &cancellables)
        ...
        NotificationCenter.default.publisher(for: .historySet)
            .sink { [weak self] output in
                ...
                Task { @MainActor in
                    let chapterCacheKey = item.chapter.sourceKey + "." + item.chapter.mangaKey + "." + item.chapter.chapterKey
                    if
                        self.chapterCache[chapterCacheKey] != nil
                            || self.missingMangaQueue[MangaKey(sourceId: item.chapter.sourceKey, mangaId: item.chapter.mangaKey)] != nil
                    {
...
extension HistoryView.ViewModel {
    ...
    private func loadMangaAndChapters(mangaKey: MangaKey, chapterIds: Set<String>) async {
        ...
        let tempManga = ExtensionRunner.Manga(sourceKey: sourceId, key: mangaId, title: "")
...
extension HistoryView.ViewModel {
    ...
    private nonisolated func processHistoryObjects(limit: Int, offset: Int) async -> Int {
        ...
        var newMangaCacheItems: [String: ExtensionRunner.Manga] = [:]
        var newChapterCacheItems: [String: ExtensionRunner.Chapter] = [:]
...
extension HistoryView.ViewModel {
    ...
    private func addMangaCacheItems(_ newItems: [String: ExtensionRunner.Manga]) {
        for (key, manga) in newItems {
            mangaCache[key] = manga
        }
    }

    private func addChapterCacheItems(_ newItems: [String: ExtensionRunner.Chapter]) {
