...
extension KavitaSeries {
    func intoManga(
        ...
        let status: ExtensionRunner.PublishingStatus = switch metadata?.publicationStatus {
        ...
        let contentRating: ExtensionRunner.ContentRating = if let ageRating = metadata?.ageRating {
            if ageRating >= 10 {
                // mature 17+ or r18
                .nsfw
            } else if ageRating >= 8 {
...
extension KavitaVolume {
    func intoChapters(baseUrl: URL, apiKey: String) -> [ExtensionRunner.Chapter] {
        chapters.compactMap { chapter -> ExtensionRunner.Chapter? in
