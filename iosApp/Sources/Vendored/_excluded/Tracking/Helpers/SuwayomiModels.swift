...
struct SuwayomiChapterNode: Decodable, Sendable {
    ...
    func intoChapter(baseUrl: URL) -> ExtensionRunner.Chapter {
        .init(
            key: "\(id)",
...
struct SuwayomiMangaNode: Decodable, Sendable {
    ...
    func intoManga(sourceKey: String, baseUrl: URL) -> ExtensionRunner.Manga? {
        guard let id, let title else { return nil }

        let status: ExtensionRunner.PublishingStatus = switch status {
            case "ONGOING": .ongoing
