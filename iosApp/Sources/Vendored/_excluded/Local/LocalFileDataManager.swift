...
extension LocalFileDataManager {
    ...
    func fetchLocalSeries(query: String? = nil) -> [ExtensionRunner.Manga] {
        let request = MangaObject.fetchRequest()
        request.predicate = if let query, !query.isEmpty {
            NSPredicate(format: "sourceId == %@ AND title CONTAINS[cd] %@", LocalSourceRunner.sourceKey, query)
        } else {
    ...
    func fetchLocalSeries(id: String) -> ExtensionRunner.Manga? {
        let request = MangaObject.fetchRequest()
        request.predicate = NSPredicate(format: "id == %@ AND sourceId == %@", id, LocalSourceRunner.sourceKey)
        request.fetchLimit = 1
        do {
    ...
    func fetchChapters(mangaId: String) -> [ExtensionRunner.Chapter] {
        let request = ChapterObject.fetchRequest()
...
extension LocalFileDataManager {
    func createManga(
        ...
        viewer: ExtensionRunner.Viewer = .unknown,
        ...
        if let comicInfo {
            let basicManga = ExtensionRunner.Manga(
    ...
    func createChapter(
        ...
        if let comicInfo {
            let basicChapter = ExtensionRunner.Chapter(
                key: id,
                title: title,
