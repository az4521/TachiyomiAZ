...
extension LocalFileManager {
    ...
    func fetchPages(mangaId: String, chapterId: String) async -> [ExtensionRunner.Page] {
    ...
    nonisolated func readEpubPages(from archiveURL: URL, chapterId: String) -> [ExtensionRunner.Page] {
        ...
        if hasText {
            ...
            return [ExtensionRunner.Page(content: .text(parts.joined(separator: "\n\n")))]
        }

        // image-only chapter (cover, illustration pages)
        return segments.compactMap { segment in
            guard case let .image(path) = segment else { return nil }
            return ExtensionRunner.Page(content: .zipFile(url: archiveURL, filePath: path))
        }
    }
