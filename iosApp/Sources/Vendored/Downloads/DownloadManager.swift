...
actor DownloadManager {
    ...
    func getDownloadedPages(for chapter: ChapterIdentifier) async -> [ExtensionRunner.Page] {
        ...
        if archiveURL.exists {
            ...
            var pages = directory.contents
                ...
                .compactMap { url -> ExtensionRunner.Page? in
                    ...
                    if LocalFileManager.allowedTextExtensions.contains(url.pathExtension) {
                        ...
                        return ExtensionRunner.Page(content: .text(text))
                    } else if LocalFileManager.allowedImageExtensions.contains(url.pathExtension) {
                        // load file as image
                        return ExtensionRunner.Page(content: .url(url: url, context: nil))
                    } else {
                        return nil
                    }
                }
...
extension DownloadManager {
    ...
    func download(manga: ExtensionRunner.Manga, chapters: [ExtensionRunner.Chapter]) async {
...
extension DownloadManager {
    ...
    func saveChapterMetadata(manga: ExtensionRunner.Manga, chapter: ExtensionRunner.Chapter, to directory: URL) {
