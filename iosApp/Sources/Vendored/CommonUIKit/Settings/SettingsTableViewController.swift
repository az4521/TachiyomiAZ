...
class SettingsTableViewController: UITableViewController {
    ...
    var source: ExtensionRunner.Source?
    ...
    init(items: [SettingItem] = [], source: ExtensionRunner.Source? = nil, style: UITableView.Style = .insetGrouped) {
        self.items = items
        self.source = source
...
// MARK: - Table View Data ExtensionRunner.Source
            ...
            case "login":
                ...
                if UserDefaults.standard.string(forKey: key) != nil { // log out
                    ...
                    let session = ASWebAuthenticationSession(url: url, callbackURLScheme: "aidoku") { callbackURL, error in
                        if let error = error {
                            let sourceInfoString = self.source != nil ? " for \(self.source?.name ?? "source")" : ""
